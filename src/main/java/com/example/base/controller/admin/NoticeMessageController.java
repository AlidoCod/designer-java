...
public class NoticeMessageController {
    ...
    final NoticeMessageService messageService;

    @Aggregation(path = "broadcast-message/publish", method = RequestMethod.POST, summary = "发布广播消息，系统通知所有用户")
    public Result publishBroadcastMessage(@RequestBody @Valid SysBroadcastMessageDto messageDto) {
    ...
    public Result queryAllBroadcastMessage(@RequestBody @Valid BasePage basePage) {
        Page<SysBroadcastMessage> sysBroadcastMessagePage = messageService.queryAllBroadcastMessage(basePage);
        return Result.data(sysBroadcastMessagePage.getRecords());
    }

    @Aggregation(path = "broadcast-message/update", method = RequestMethod.POST, summary = "修改广播消息")
    public Result updateBroadcastMessage(@RequestBody @Valid UpdateSysBroadcastMessageDto messageDto) {
